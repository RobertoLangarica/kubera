...
public class AudioObject : CachedMonoBehaviour 
{
    ...
    public void Play()
    {
        if (currentClip.clip != null)
        {
            ...
            if (!mustLoop)
            {
				...
				if (currentSubItem.RelatedAudioItem.mustShowDebugInfo)
					Debug.Log("AutoStop in["+stopInSeconds+"] at["+(Time.realtimeSinceStartup + stopInSeconds )+"] because [" + stopInSeconds + "] + now["+Time.realtimeSinceStartup+"] - fadeOut["+fadeOutDuration+"]");
    ...
    */
	...
	{
		if (currentSubItem.RelatedAudioItem.mustShowDebugInfo && !isPaused)
			Debug.Log("AutoStopping["+name+"]["+instanceId+"]["+currentSubItem.RelatedAudioItem.itemId+"] at["+Time.realtimeSinceStartup+"]");
		Stop();
	}
    ...
    */
    public void DestroyAudioObject()
    {
        ...
        {
            ObjectManager.Despawn(CachedAudioPooledObject);
        }
        else
        {
